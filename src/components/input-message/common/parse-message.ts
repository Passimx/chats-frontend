...
export const parseMessage = (text: string): ParseMessagePartType[] => {
    ...
    const nameRegex: RegExp = /@[\p{L}\p{N}_-]+/gu;

    const parts: ParseMessagePartType[] = [];
    let lastIndex = 0;

    text?.replace(
        new RegExp(`${domainRegex.source}|${hashtagRegex.source}|${nameRegex.source}`, 'gu'),
        (match, offset) => {
            if (offset > lastIndex) {
            ...
            if (match.startsWith('#')) {
                parts.push({ type: PartTypeEnum.TAG, content: match });
            } else if (match.startsWith('@')) {
                parts.push({ type: PartTypeEnum.NAME, content: match.slice(1) });
            } else {
